...
public partial class MemoryPriorityViewModel : ObservableObject
{
    ...
    public async Task<string> ApplyRecommendedLimitsAsync()
    {
        ...
        var planned = await Task.Run(() =>
        {
            ...
            {
                // Shared brain with the engine's automatic assignment: measured when
                // running, curated catalog when not, null when there's no honest basis.
                var mb = RecommendedLimits.RecommendMb(vm.ExePath, _priority);
                var option = PriorityRuleVm.MemoryLimitOptions.FirstOrDefault(o => o.Mb == mb);
                if (mb is null || option is null) continue;
